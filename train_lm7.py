...
x = [seq[:-1] for seq in idx_seven_seq]
y = [seq[1:] for seq in idx_seven_seq]
x_train = torch.tensor(x[:500000], dtype = torch.long)
y_train = torch.tensor(y[:500000], dtype = torch.long)
train_dataset = TensorDataset(x_train, y_train)
train_loader = DataLoader(train_dataset,batch_size = batch_size,shuffle=True)
x_test = torch.tensor(x[500000:], dtype = torch.long)
y_test = torch.tensor(y[500000:], dtype = torch.long)
...
for epoch in range(num_epochs):
    ...
    if len(val_losses) == 0 or val_loss < min(val_losses):
        ...
        torch.save(model.state_dict(), "model/lm7_best_myself.th")
