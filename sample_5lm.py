...
model.cuda()
model.load_state_dict(torch.load('model/lm5_best_2.th'))
...
for i in range (100):  
    with torch.no_grad():
        ...
        for i in range(13):
            ...
            bos = bos.cuda()
            example.append(idx)
    five_generator.append([idx2word[word_idx] for word_idx in example])
five_generator = [''.join(seq) for seq in five_generator]

file = open('output/sampled_five_sentences.txt','w',encoding='UTF-8');
for i, sentence in enumerate(five_generator):
