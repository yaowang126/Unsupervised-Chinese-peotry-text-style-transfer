# -*- coding: utf-8 -*-
"""
Created on Sun Dec 20 19:51:20 2020
...
model57.cuda()
model75.cuda()
model75.load_state_dict(torch.load('model/model75_att_t=0.1_cross%kl=100.pt'))

test = idx_seven_seq[round(np.random.rand()*len(idx_seven_seq))-1]
test = torch.tensor(test, dtype = torch.long,requires_grad=False)
test = test.unsqueeze(0).cuda()
output75, tryonehot75, tryprob75 = model75(test,gumbel=1)
output5s = []
for i in range(10):
    inputs_grad75 = gumbel_softmax(output75,0.1 )
    output5 = torch.argmax(inputs_grad75,dim=2,keepdim=False)
    output5 = output5.squeeze(0).cpu().numpy()
    trans5 = ''.join([idx2word[i] for i in output5])
    output5s.append(trans5)


file = open('output/sampled_seven2five.txt','w',encoding='UTF-8')
test = test.squeeze(0).cpu().numpy()
file.write('The input seven sentence is')
file.write('\t')
file.write(''.join([idx2word[i] for i in test]))
file.write('\n')
file.write('The input five sentence can be')
file.write('\n')
for i, sentence in enumerate(output5s):
